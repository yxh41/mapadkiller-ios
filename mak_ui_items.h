// 自动生成文件 —— 由 re/gen_ui.py 生成，请勿手改。
// 数据源同 Preferences/Resources/Root.plist：同一份 SPEC 表多个产物，避免漂移。
// 锚点来自真机日志 UI TXT / UI CLS（高德 iOS 15.x 原生视图树）。
#ifndef MAK_UI_ITEMS
#define MAK_UI_ITEMS \
    @{    @"ui_tab_home": @[@"首页"],\
    @"ui_tab_explore": @[@"附近", @"探索"],\
    @"ui_tab_voice": @[@"语音输入", @"长按说话"],\
    @"ui_tab_taxi": @[@"打车"],\
    @"ui_tab_message": @[@"消息"],\
    @"ui_tab_mine": @[@"我的"],\
    @"ui_search_scan": @[@"扫一扫"],\
    @"ui_search_voice": @[@"语音输入"],\
    @"ui_tool_drive": @[@"驾车"],\
    @"ui_tool_bus": @[@"公交地铁", @"公交", @"实时公交"],\
    @"ui_tool_rental": @[@"租车"],\
    @"ui_tool_taxi": @[@"打车"],\
    @"ui_tool_hotel": @[@"订酒店"],\
    @"ui_tool_train": @[@"火车票机票", @"火车票"],\
    @"ui_tool_carpool": @[@"顺风车"],\
    @"ui_tool_scanstreet": @[@"高德扫街"],\
    @"ui_tool_daijia": @[@"代驾"],\
    @"ui_tool_gas": @[@"优惠加油"],\
    @"ui_tool_express": @[@"秒送"],\
    @"ui_tool_more": @[@"更多工具"],\
    @"ui_tool_extra_page": @[@"通行费助手", @"离线地图", @"景点游玩", @"旅游度假", @"实时公交", @"领油券"],\
    @"ui_home_shortcut": @[@"回家", @"去单位", @"去设置"],\
    @"ui_map_route": @[@"路线"],\
    @"ui_map_more": @[@"更多"],\
    @"ui_map_layers": @[@"图层"],\
    @"ui_map_locate": @[@"我的位置"],\
    @"ui_map_attribution": @[@"高德图强提供技术支持", @"协议中心-", @"-资质信息"],\
    @"ui_my_order_row": @[@"全部订单", @"待付款", @"待使用", @"待评价", @"退款/取消"],\
    @"ui_my_service_row": @[@"我的车辆", @"我要卖车", @"洗车", @"租车", @"加油、洗车优惠点这里", @"高德油耗"],\
    @"ui_my_task": @[@"立即加入", @"您将成为Lv.2级达人", @"建设高德地图需要你", @"点击查看", @"达人权益中心", @"做达人，免费领大额权益", @"达人卡兑换"],\
    @"ui_my_promo_row": @[@"蚂蚁森林", @"领勋章", @"3小时公益", @"高德公益", @"拍图有奖", @"问答有礼", @"游戏中心", @"领航员小屋", @"地图大富翁", @"长征星火", @"高德快报", @"赢奖牌", @"每日签到", @"小德果园", @"互动专区"],\
    @"ui_my_guess": @[@"超前科技 全域覆盖", @"V15新版本六大更新", @"v15功能3D立体地图", @"城市动态 一目了然", @"查看你的年度回忆", @"2024年度报告来了！", @"新开启雪天、临崖路段安全预警", @"安全预警再升级", @"路口听导航，少等红灯多省油", @"创新首发：红绿灯AI领航", @"出行黑科技", @"带娃必备，大小朋友都爱", @"『超级飞侠』乐迪来了", @"超甜超甜随机粤语导航语音", @"广式女友小葵总", @"甜美御姐双面阿七上线", @"你的导航电子狗来了", @"李宗恒舔狗导航上新！", @"俺叫熊大，俺带你去狗熊岭瞧瞧？", @"熊出没主题导航上线！", @"限时福利，手慢无！", @"好礼每日刷新~限量手慢无", @"邀好友天天0元领福利！", @"来！吃喝玩乐超划算~", @"0元定制足迹礼物", @"包含加油/美食/酒店专享券", @"全网比价", @"放心囤"],\
    @"ui_my_social": @[@"我的反馈", @"我的相册", @"群组", @"家人地图"],\
    @"ui_my_wallet": @[@"钱包卡券", @"收藏夹", @"我的店铺", @"地图小程序"],\
    @"ui_my_decor": @[@"经典主题", @"皮肤", @"3D跑车", @"车标", @"语音包", @"小团团"],\
    @"ui_my_stats": @[@"获赞总数", @"浏览总量", @"帮助同行人", @"贡献总数", @"点亮城市", @"出行里程", @"打卡点", @"足迹", @"主页", @"关注"],\
    @"ui_my_settings": @[@"设置", @"常去地点", @"添加常去地点", @"去单位去设置", @"回家 去设置"],\
    @"ui_feed_taxi": @[@"同城跑腿秒送达", @"取送东西选秒送", @"接车保镖 现已上线", @"1分起叫代驾", @"高德代驾，抢神券", @"打车更轻松", @"去打车", @"行程有保障", @"限时特惠", @"去体验", @"最高减10元", @"首单立享1折", @"小雷达探索中", @"设置常去地点"],\
    @"ui_feed_group": @[@"超划算·限量特惠", @"已售4万+", @"已售6000+", @"已售3万+", @"【升级香辣鸡腿堡】单人四件餐", @"秘汁手扒鸡TG031", @"【3周年同庆】金奖美式4选1"]}
#endif

#ifndef MAK_UI_CLASS_ITEMS
#define MAK_UI_CLASS_ITEMS \
    @{    @"uic_toolbox": @[@"GDLiteToolBoxView"],\
    @"uic_home_company": @[@"GDLiteHomeCompanyView"],\
    @"uic_searchbar": @[@"WINQuickSearchBarV2"],\
    @"uic_map_widget": @[@"MapWidgetContainerView"],\
    @"uic_tabbar": @[@"WINTabBar"],\
    @"uic_feed_promo": @[@"WINOperationWidgetView"]}
#endif

#ifndef MAK_UI_DEFAULTS
#define MAK_UI_DEFAULTS \
    @{@"ui_tab_home": @YES, @"ui_tab_explore": @YES, @"ui_tab_voice": @YES, @"ui_tab_taxi": @YES, @"ui_tab_message": @YES, @"ui_tab_mine": @YES, @"ui_search_scan": @YES, @"ui_search_voice": @YES, @"ui_tool_drive": @YES, @"ui_tool_bus": @YES, @"ui_tool_rental": @YES, @"ui_tool_taxi": @YES, @"ui_tool_hotel": @YES, @"ui_tool_train": @YES, @"ui_tool_carpool": @YES, @"ui_tool_scanstreet": @YES, @"ui_tool_daijia": @YES, @"ui_tool_gas": @YES, @"ui_tool_express": @YES, @"ui_tool_more": @YES, @"ui_tool_extra_page": @YES, @"ui_home_shortcut": @YES, @"ui_map_route": @YES, @"ui_map_more": @YES, @"ui_map_layers": @YES, @"ui_map_locate": @YES, @"ui_map_attribution": @YES, @"uic_toolbox": @YES, @"uic_home_company": @YES, @"uic_searchbar": @YES, @"uic_map_widget": @YES, @"uic_tabbar": @YES, @"uic_feed_promo": @NO, @"ui_my_order_row": @YES, @"ui_my_service_row": @YES, @"ui_my_task": @YES, @"ui_my_promo_row": @YES, @"ui_my_guess": @YES, @"ui_my_social": @YES, @"ui_my_wallet": @YES, @"ui_my_decor": @YES, @"ui_my_stats": @YES, @"ui_my_settings": @YES, @"ui_feed_taxi": @YES, @"ui_feed_group": @YES}
#endif
