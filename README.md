...
|---|---|---|---|
| L1 | `ViewKiller`（遍历 DecorView 隐藏/移除广告 View） | `sweepView()`：`UIViewController -viewDidAppear:` 触发，递归 subviews，按类名 / `accessibilityLabel` / `accessibilityIdentifier` 命中即 `hidden + removeFromSuperview` | ✅ 通用，无需逆向 |
...
因此偏好目前走**编译期默认值 + fail-open**：

| 开关 | 默认值 | 说明 |
|---|---|---|
| `Enabled` / `ViewSweep` / `SdkBlock` | YES | 三层默认全开 |
| `Amap` / `Bmap` / `Tmap` | YES | 三个目标 App 全启用 |
| `Aggressive` | **NO** | 见下 |
| `DebugLog` | NO | 开了才打日志 |

只要 dylib 注入成功，即使一个字节的偏好都没读到，去广告也是全开的。
运行时会尝试从 `/var/mobile/Library/Preferences/com.yxh41.mapadkiller.plist` 读一次；
roothide 下 App Store App 通常读不到，读不到就保持默认值。

> 设置面板要回来，得先解决 roothide 下的跨进程偏好同步（Cephei 干的就是这件事）。
> 在那之前面板只是"看起来能改但实际无效"，所以没放上去误导人。
...
Tweak.xm                 三层实现 + 高德 hook 列表
re/ipa_dump.py           离线静态取证（纯标准库，推荐）
re/dump_classes.js       Frida 备用方案
re/dump_views.js         Frida 备用方案
re/static_dump.py        更早期的单 Mach-O 解析器，已被 ipa_dump.py 取代
re/RE-GUIDE.md           取证指南
re/amap_1503.0_dump.txt  高德 15.03.0 的 dump 结果
```
